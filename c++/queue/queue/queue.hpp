...
node::node() :
		next(nullptr), data(NAN)
{
}
;
node::node(double __data) :
		next(nullptr), data(__data)
{
}
;
node::node(double __data, node *__next) :
...
void queue::erase(double __data) //again, bit different from dlinked case
{
	...
	if (p != nullptr)
	{
		qsize--;
		node *hb = head;
		while (hb->next != p && hb->next != nullptr)
			hb = hb->next;
		//find element immediately before p

		if (p == head)
			head = p->next; //handle head of list
		else if (p->next == nullptr)
			hb->next = nullptr;  // handle tail of list
		else
			hb->next = p->next;  // handle inside of list
	}
}
